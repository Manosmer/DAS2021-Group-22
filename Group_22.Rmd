...
Throughout the report, we consider various numerical and graphical summaries, followed by the use of an appropriate generalised linear model in order to assess the relationship between the available variables and the 1000 Suadi Riyals threshold related to the price(Osborne,2017).
...
## Variable Selection
At the beginning, we will add all variables except "sellable_online" to the model(Tutz,2012). The model will be as follows:
...
```
It can be seen that except for the variable "other_colors", all confidence intervals do not include 0. Therefore, "other_colors" cannot be considered significant. The high p-value can double-check the result(Dippold and Hruschka, 2013).
```{r}
summary(model1) %>%
coefficients %>%
...
[1]Osborne, J. (2017). Regression & linear modeling. SAGE Publications, Inc https://www-doi-org.ezproxy.lib.gla.ac.uk/10.4135/9781071802724​

[2] Tutz, G. (2012). Regression for categorical data. Cambridge University Press.​

[3] Dippold, K., Hruschka, H. Variable selection for market basket analysis. Comput Stat 28, 519–539 (2013). https://doi-org.ezproxy.lib.gla.ac.uk/10.1007/s00180-012-0315-3
